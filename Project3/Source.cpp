...
void upisUDatotekuPacijenata(string name, string prezime, string bolest, int brojKreveta, int brojDanaUBolnici, char izbor, char uputnica)
{
	...
	if (out) {
		...
		{
			cout << "---------------------------------------------" << endl;
			cout << "Termin zakazan za : " << (izbor - 65) + 9 << " uspjesno !!";
		}
		else if (uputnica == 'N' || uputnica == 'n')
		{
			cout << "------------------------------------------------------------------" << endl;
			cout << "\n Termin zakazan za : " << (izbor - 65) + 9 + 2 << " uspjesno (bez uputnice termin koji ste zakazali ste morali naplatiti 10 KM)";
		}
...
int zakazatiTermin()
{
	...
	if (pronadjenTermin == 0) {
		cout << "\t\t\t\t\t";
		cout << "    Termin slobodan za iduce sate :";
		char termin = 'A';
		cout << "\t\t\t\t\t\t\t";
		for (int i = 9; i <= 21; i++)
		{
